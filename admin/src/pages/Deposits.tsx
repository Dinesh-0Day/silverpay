...
import { adminApi, type Deposit, type PaginationMeta, getErrorMessage } from "../api";
import {
  PageHeader,
  Card,
  Badge,
...
export default function Deposits() {
  ...
  const load = (p = page) => {
    ...
    setError("");
    return adminApi
      .deposits({ status: filter || undefined, page: p, limit: PAGE_SIZE })
      .then((res) => {
        setList(res.items);
  ...
  const pendingCount = filter === "PENDING" ? pagination?.total : undefined;
